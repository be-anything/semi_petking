...
document.addEventListener('submit', (e) => {
    ...
    if(e.target.matches("[name=clubCommentCreateFrm]")){
        const frm = e.target;
...
document.querySelectorAll(".btn-reply").forEach((button)=>{
    button.addEventListener('click', (e)=>{
        ...
        const {contextPath, clubId, loginUserId} = e.target.dataset;
        // 대댓글 입력폼 tr
        const html = `
              <tr>
                <td colspan="4">
                  <form name="clubCommentCreateFrm" action="${contextPath}/club/clubCommentCreate" method="post">
