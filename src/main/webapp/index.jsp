...
<div class="h-[600px] w-full bg-gray1" style="background-image: url('${pageContext.request.contextPath}/images/common/index_bg.jpg'); background-size: cover; background-position: center">
    <div class="pt-80 ">
        <%--검색하기--%>
        <form name="campSearchFrm" action="${pageContext.request.contextPath}/camp/campList" method="get" class="pt-4 bg-[#FEEBB6]/90 mt-24">
        <div class="mx-auto max-w-6xl w-fit">
            <div class="flex">
                ...
                <div class="ml-1">
                    <input type="search" id="search-keyword" name="search-keyword" value="${param['search-keyword']}" placeholder="검색어를 입력하세요..." class="text-black placeholder:text-white border border-pink bg-light-pink font-medium rounded-lg text-sm py-2.5 px-5 w-[500px]" >
                    <button type="submit" class="search-btn text-white end-2.5 bottom-2.5 bg-pink hover:bg-black hover:text-white focus:outline-none font-medium rounded-lg text-sm px-4 py-3">검색하기</button>
                </div>
            </div>
        </div>
        <div class="px-4 py-6 sm:grid sm:grid-cols-1 sm:gap-4 sm:px-0">
            <div class="w-3/4 mx-auto px-4 py-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-0">
                <c:forEach items="${tags}" var="tag" varStatus="vs">
                    <div class="options grid w-full gap-6 text-gray-500 border-2 bg-white/80 border-white rounded-lg cursor-pointer inline-flex items-center justify-center w-full p-3">
...
<div class="text-center my-24 pt-32">
